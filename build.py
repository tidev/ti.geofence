...
def create_module(platform, cmd):
    # Directories to symlink in each platfrom pointing to directories in the module root
    dirs_to_link = ['example', 'documentation']
    build_path = os.path.join(os.getcwd(), platform)
    # Add symlinks
    for cur_dir in dirs_to_link:
        os.symlink(os.path.join(os.getcwd(),cur_dir), os.path.join(build_path, cur_dir))

    retcode = fork(build_path, cmd, False)
    # Remove syminks
    for cur_dir in dirs_to_link:
        os.remove(os.path.join(build_path, cur_dir))
